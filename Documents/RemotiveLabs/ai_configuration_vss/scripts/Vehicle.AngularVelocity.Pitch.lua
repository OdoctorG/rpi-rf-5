local local_signals = {{
    name = "ID101RCM_inertial1.RCM_pitchRate",
...
function output_signal()
    -- remianing meters
    return "Vehicle.AngularVelocity.Pitch"
end
...
function signals(signals, namespace, signals_timestamp_us, system_timestamp_us)
    ...
    -- Check that signal is not "SNA"
    -- RCM_pitchRate 16384 "SNA"
    if (signals["ID101RCM_inertial1.RCM_pitchRate"] ~= nil and signals["ID101RCM_inertial1.RCM_pitchRate"] ~= 16384*0.00025) then
        return return_value_or_bytes(signals["ID101RCM_inertial1.RCM_pitchRate"] * 57.2957795)
    else
        return return_nothing()
    end
end
