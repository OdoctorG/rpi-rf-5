local local_signals = {{
    name = "ID3C2VCLEFT_switchStatus.VCLEFT_brakePressed",
...
function signals(signals, namespace, signals_timestamp_us, system_timestamp_us)
    ...
    if (signals["ID3C2VCLEFT_switchStatus.VCLEFT_brakePressed"] ~= nil) then
        return return_value_or_bytes(signals["ID3C2VCLEFT_switchStatus.VCLEFT_brakePressed"] * 100) -- convert to percent
    else
        return return_nothing()
    end
end
