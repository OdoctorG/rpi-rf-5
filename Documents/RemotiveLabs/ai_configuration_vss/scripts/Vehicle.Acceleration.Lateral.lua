local local_signals = {{
    name = "ID111RCM_inertial2.RCM_lateralAccel",
...
function output_signal()
    return "Vehicle.Acceleration.Lateral"
end
...
function signals(signals, namespace, signals_timestamp_us, system_timestamp_us)
    ...
    if (signals["ID111RCM_inertial2.RCM_lateralAccel"] ~= nil and signals["ID111RCM_inertial2.RCM_lateralAccel"] ~= 32768*0.00125) then
        return return_value_or_bytes(signals["ID111RCM_inertial2.RCM_lateralAccel"])
    else
        return return_nothing()
    end
end
